...
class AudioRouting {
  ...
  static void configureForSpeakerOutput() {
    try {
      web.console.log(
          '[AudioRouting] Configuring audio for speaker output on web'.toJS);
      ...
      try {
        ...
        web.console
            .log('[AudioRouting] MediaSession configured successfully'.toJS);
      } catch (e) {
        web.console.error(
            '[AudioRouting] Failed to set MediaSession metadata: $e'.toJS);
      }
      ...
      try {
        ...
        web.console
            .log('[AudioRouting] AudioContext created for media routing'.toJS);

        // Step 3: Attempt to explicitly route audio to speaker device
        _trySetSpeakerOutput(context);
      } catch (e) {
        web.console
            .log('[AudioRouting] Could not create AudioContext: $e'.toJS);
      }
    } catch (e) {
      web.console
          .error('[AudioRouting] Error configuring speaker output: $e'.toJS);
    }
  }
      ...
      try {
        ...
        if (sinkIdPromise != null) {
          sinkIdPromise.then(
            (value) {
              web.console
                  .log('[AudioRouting] Audio sink set to default speaker'.toJS);
            },
            (error) {
              web.console
                  .log('[AudioRouting] Could not set audio sink: $error'.toJS);
            },
          );
        }
      } catch (e) {
